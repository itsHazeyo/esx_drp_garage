...
function reparation(prix, vehicle, vehicleProps)
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'delete_menu', {
        ...
        if (data.current.value == 'yes') then
            TriggerServerEvent('esx_garage:payhealth', prix)
...
RegisterNetEvent('esx_garage:deletevehicle_cl')

AddEventHandler('esx_garage:deletevehicle_cl', function(plate)
...
function ranger(vehicle, vehicleProps)
    TriggerServerEvent('esx_garage:deletevehicle_sv', vehicleProps.plate)
    TriggerServerEvent('esx_garage:modifystate', vehicleProps, true)
...
function StockVehicleMenu()
    ...
    if IsPedInAnyVehicle(playerPed, false) then
        ...
        ESX.TriggerServerCallback('esx_garage:stockv', function(valid)
            if (valid) then
                ESX.TriggerServerCallback('esx_garage:getVehicles', function(vehicules)
                    ...
                    for _, v in pairs(vehicules) do
                        if plate == v.plate then
                            owned = true
                            TriggerServerEvent('esx_garage:debug', 'vehicle plate returned to the garage: ' .. vehicleProps.plate)
                            TriggerServerEvent('esx_garage:logging', 'vehicle returned to the garage: ' .. engineHealth)
...
function SpawnVehicle(vehicle)
    ...
    TriggerServerEvent('esx_garage:modifystate', vehicle, false)
end
...
function SpawnPoundedVehicle(vehicle)
    ...
    TriggerServerEvent('esx_garage:modifystate', vehicle, true)
...
function ReturnVehicleMenu()
    ESX.TriggerServerCallback('esx_garage:getOutVehicles', function(vehicles)
        local elements = {}
        ...
        ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'return_vehicle', {
            ...
            ESX.TriggerServerCallback('esx_garage:checkMoney', function(hasEnoughMoney)
                if hasEnoughMoney then
                    if times == 0 then
                        TriggerServerEvent('esx_garage:pay')
